...
if [ ! -f /var/www/html/config.php ]; then
    ...
    # Dual-host wwwroot: Moodle risponde sia su localhost:8080 (browser) sia
    # su vektra-moodle43 (rete Docker, n8n). Sostituisce la wwwroot statica
    # con una dinamica basata su HTTP_HOST.
    sed -i "s|\$CFG->wwwroot   = '${MOODLE_URL}';|\$CFG->wwwroot = 'http://' . (\$_SERVER['HTTP_HOST'] ?? '${MOODLE_URL#http://}');|" /var/www/html/config.php
